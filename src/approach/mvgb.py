...
class Appr(Inc_Learning_Appr):
    ...
    def __init__(self, model, device, nepochs=100, lr=0.05, lr_min=1e-4, lr_factor=3, lr_patience=5, clipgrad=10000,
                 momentum=0, wd=0, multi_softmax=False, wu_nepochs=0, wu_lr_factor=1, patience=5, fix_bn=False,
                 eval_on_train=False,
                 logger=None, gmms=1, use_multivariate=True, use_head=False, remove_outliers=False,
                 load_distributions=False, save_distributions=False):
        ...
        with torch.no_grad():
            ...
            for c in range(classes):
                ...
                if isinstance(trn_loader.dataset.images, list):
                    ...
                    ds = np.concatenate(
                        (trn_loader.dataset.images[train_indices], val_loader.dataset.images[val_indices]), axis=0)
                ...
                while not is_ok:
                    try:
                        ...
                        eps = 10 * eps
                        print(
                            f"WARNING: Covariance matrix is singular. Compensation initialized. Changing eps to: {eps:.8f}")
                    else:
                        is_ok = True
    ...
    def predict_class_bayes(self, features):
        with torch.no_grad():
            log_probs = [self.task_distributions[t].score_samples(features) for t in
                         range(len(self.task_distributions))]
            log_probs = torch.stack(log_probs, dim=1)
