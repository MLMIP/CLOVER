...
def get_data(trn_data, tst_data, num_tasks, nc_first_task, validation, shuffle_classes, class_order=None):
    ...
    if class_order is None:
        ...
        class_order = class_order.copy()  # class_order是固定的, 是seed=0时的序列
    if shuffle_classes:
        np.random.shuffle(class_order)
        # np.random.seed(1993)
        # class_order = np.random.permutation(len(class_order)).tolist()
    ...
    # ALL OR TRAIN, 将训练集数据记录到data[tt]['trn']中
    ...
    for this_image, this_label in zip(trn_data['x'], trn_data['y']):  # 遍历训练集
        # If shuffling is false, it won't change the class number
        this_label = class_order.index(this_label)  # 获取打乱后的标签
        # add it to the corresponding split
        this_task = (this_label >= cpertask_cumsum).sum()  # 获取task-id
        data[this_task]['trn']['x'].append(this_image)
        data[this_task]['trn']['y'].append(this_label - init_class[this_task])  # 将所有类标签限定在任务内(0~9)

    # ALL OR TEST, 将测试集数据记录到data[tt]['tst']中
    ...
    # check classes
    for tt in range(num_tasks):
        data[tt]['ncla'] = len(np.unique(data[tt]['trn']['y']))  # 记录每个Task所含的类数
    ...
    for tt in data.keys():
        for split in ['trn', 'val', 'tst']:
            data[tt][split]['x'] = np.asarray(data[tt][split]['x'])  # 将数据改成ndarray对象
