...
from .base_dataset import DataManager, DataManager_feCAM, DataManager_imagenet1k
...
def get_loaders(datasets, num_tasks, nc_first_task, batch_size, num_workers, pin_memory, validation=.1,
    ...
    trn_load, val_load, tst_load, all_tst_load = [], [], [], []
    taskcla = []
    dataset_offset = 0
    for idx_dataset, cur_dataset in enumerate(datasets, 0):
        ...
        if cur_dataset in ['cifar100_icarl']:
            trn_transform = transforms.Compose([transforms.RandomCrop((32, 32), padding=4),
            ...
            tst_transform = transforms.Compose([transforms.ToTensor(),
                                                transforms.Normalize((0.5071, 0.4867, 0.4408),
                                                                     (0.2675, 0.2565, 0.2761))])
        elif cur_dataset in ['cifar100_fecam']:
        ...
        elif cur_dataset in ['imagenet_subset_fecam']:
            trn_transform = transforms.Compose([
        ...
        elif cur_dataset in ['tinyImagenet200']:
            trn_transform = transforms.Compose([
                transforms.RandomHorizontalFlip(),
                transforms.ToTensor(),
                transforms.Normalize(mean=[0.485, 0.456, 0.406],
        ...
        elif cur_dataset in ['tinyImagenet200_fecam']:
            trn_transform = transforms.Compose([
                transforms.RandomCrop(32, padding=4),
                transforms.RandomHorizontalFlip(),
                transforms.ToTensor(),
        ...
        # datasets
        trn_dset, val_dset, tst_dset, all_tst_dset, curtaskcla = get_datasets(cur_dataset, dc['path'], num_tasks,
                                                                              nc_first_task,
                                                                              # dc['path']='../data/cifar100'; num_tasks=10; nc_first_task=10
                                                                              validation=validation,
                                                                              trn_transform=trn_transform,
                                                                              tst_transform=tst_transform,
                                                                              class_order=dc['class_order'])

        # apply offsets in case of multiple datasets; 用于DIL，这里用不上
        if idx_dataset > 0:
            for tt in range(num_tasks):  # dataset_offset是用于同时预测多个数据集时，隔开标签id用的
                ...
                all_tst_dset[tt].labels = [elem + dataset_offset for elem in all_tst_dset[tt].labels]
        dataset_offset = dataset_offset + sum([tc[1] for tc in curtaskcla])

        # reassign class idx for multiple dataset case; [(0, 10), (1, 10), (2, 10), (3, 10), (4, 10), (5, 10), (6, 10), (7, 10), (8, 10), (9, 10)]
        ...
        for tt in range(num_tasks):
            ...
            tst_load.append(data.DataLoader(tst_dset[tt], batch_size=batch_size, shuffle=False, num_workers=num_workers,
                                            pin_memory=pin_memory))
            if len(all_tst_dset) != 0:
                all_tst_load.append(data.DataLoader(all_tst_dset[tt], batch_size=batch_size, shuffle=False,
                                                    num_workers=num_workers, pin_memory=pin_memory))
    return trn_load, val_load, tst_load, all_tst_load, taskcla
...
def get_datasets(dataset, path, num_tasks, nc_first_task, validation, trn_transform, tst_transform, class_order=None):
    ...
    elif dataset in ['cifar100', 'cifar100_icarl_224', 'cifar100_fecam']:
    ...
    elif dataset in ['imagenet_subset_kaggle']:
    ...
    elif dataset in ['imagenet_1k']:
        data_manager = DataManager_imagenet1k(path)

        # 每个任务（非first task）的类别数
        per_task_classes = (1000 - nc_first_task) // (num_tasks - 1)
        taskcla = []
        for task in range(num_tasks):
            class_set = list(range(1000))
            if task == 0:
                classes = class_set[:nc_first_task]
                taskcla.append((task, nc_first_task))
            else:
                classes = class_set[nc_first_task + (task - 1) * per_task_classes:
                                    nc_first_task + task * per_task_classes]
                taskcla.append((task, per_task_classes))
            all_classes = class_set[:nc_first_task + task * per_task_classes]
            print(classes)

            trainDataset = data_manager.get_dataset(trn_transform, index=classes, train=True)
            testDataset = data_manager.get_dataset(tst_transform, index=classes, train=False)
            allTestDataset = data_manager.get_dataset(tst_transform, index=all_classes, train=False)
            trn_dset.append(trainDataset)
            val_dset.append(testDataset)
            tst_dset.append(testDataset)
            all_tst_dset.append(allTestDataset)
        return trn_dset, val_dset, tst_dset, all_tst_dset, taskcla

    elif dataset in ['imagenet_subset_fecam']:
        _ensure_imagenet_subset_prepared(path)
        # read data paths and compute splits -- path needs to have a train.txt and a test.txt with image-label pairs
        all_data, taskcla, class_indices = basedat.get_data(path, num_tasks=num_tasks, nc_first_task=nc_first_task,
                                                            validation=validation, shuffle_classes=class_order is None,
    ...
    elif dataset in ['tinyImagenet200', 'tinyImagenet200_64', 'tinyImagenet200_128', 'tinyImagenet200_224',
                     'tinyImagenet200_fecam']:
        data_manager = DataManager(path)

        per_task_classes = (200 - nc_first_task) // (num_tasks - 1)
        taskcla = []
        for task in range(num_tasks):
...
def _ensure_domainnet_prepared(path, classes_per_domain=50, num_tasks=6):
    assert os.path.exists(
        path), f"Please first download and extract dataset from: http://ai.bu.edu/M3SDA/#dataset into:{path}"
