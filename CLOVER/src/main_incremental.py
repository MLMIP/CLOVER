...
def main(argv=None):
    ...
    parser.add_argument('--loop-choose', action='store_true',
                        help='增量阶段，循环选择专家(使得每个专家训练相同的任务数以减少遗忘) (default=%(default)s)')
    parser.add_argument('--num-feature', type=int, default=0,
                        help='模型输出的特征维度,为0则表示该项无效，默认为64 (default=%(default)s)')
    parser.add_argument('--balance', type=float, default=0,
                        help='dis_loss的权值 (default=%(default)s)')
    parser.add_argument('--w0', type=float, default=0,
                        help='加权时，新专家初始的权值 (default=%(default)s)')
    parser.add_argument('--w1', type=float, default=0,
                        help='加权时，新专家递增的权值 (default=%(default)s)')
    ...
    parser.add_argument('--ft-lr', default=0.1, type=float, required=False,
                        help='Fine-tuning learning rate (default=%(default)s)')
    parser.add_argument('--ft-weight-decay', default=0.0, type=float, required=False,
                        help='Fine-tuning Weight decay (L2 penalty) (default=%(default)s)')
    ...
    if args.network in tvmodels:
        ...
        init_model = net(pretrained=False)  # resnet32
    ...
    logger = MultiLogger(args.results_path, full_exp_name, loggers=args.log, save_models=False)
    logger.log_args(argparse.Namespace(**args.__dict__, **appr_args.__dict__, **appr_exemplars_dataset_args.__dict__))

    # Loaders
    trn_loader, val_loader, tst_loader, all_tst_load, taskcla = get_loaders(args.datasets, args.num_tasks,
                                                                            args.nc_first_task,
                                                                            args.batch_size,
    ...
    # Calculate task weights by taskcla
    task_weight = [k for _, k in taskcla]
    ...
    if args.use_test_as_val:
        val_loader = tst_loader  # 将测试集当验证集
    max_task = len(taskcla) if args.stop_at_task == 0 else args.stop_at_task

    # Network and Approach instances
    if args.approach in ["clover", "seed"]:
        net = ExtractorEnsemble(init_model, taskcla, args.network, device, num_feature=args.num_feature)
    elif args.approach == 'mbm':
    ...
    appr = Appr(net, device, batch_size=args.batch_size, save_models=args.save_models, loop_choose=args.loop_choose,
                ft_lr=args.ft_lr, ft_wd=args.ft_weight_decay, dataset=args.datasets,
                taskcla=taskcla, balance=args.balance, w0=args.w0, w1=args.w1,
                **appr_kwargs)  # 将模型包装, 方便训练
    ...
    # 计算专家关于自身训练过的任务的准确率、对新旧类的准确率
    acc_tag_within_expert = [np.zeros((appr.max_experts, max_task)) for _ in range(max_task)]
    ...
    for t, (_, ncla) in enumerate(taskcla):
        ...
        net.add_head(taskcla[t][1])  # 此处没用上, 并不是动态扩展分类头
        net.to(device)
        ...
        trained_expert = appr.train(t, trn_loader[t], tst_loader[t])
        print('-' * 108)

        acc_list = []
        for i in range(len(acc_tag_within_expert[t][trained_expert])):
            if i in appr.train_history_task[trained_expert]:
                acc_list.append(acc_tag_within_expert[t][trained_expert][i])

        # Test
        for u in range(t + 1):
            test_loss, acc_taw[t, u], acc_tag[t, u] = appr.eval(u, tst_loader)
            ...
            logger.log_scalar(task=t, iter=u, name='acc_tag', group='test',
        ...
        print(
            f"acc_taw for now is: {sum([acc_taw[t, i] * task_weight[i] for i in range(t + 1)]) / sum(task_weight[:t + 1])}")
        print(
            f"acc_tag for now is: {sum([acc_tag[t, i] * task_weight[i] for i in range(t + 1)]) / sum(task_weight[:t + 1])}")

    # Print Summary

    utils.print_summary_weighted(acc_taw, acc_tag, forg_taw, forg_tag, task_weight)
